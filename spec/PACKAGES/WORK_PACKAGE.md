...
固定运行时上限按 UTF-8 原始字节计算：Manifest 256 MiB、Execution 4 MiB、配置/Prompt/`.env.example` 每项 8 MiB、Canonical Tests 1.25 GiB、Canonical Case 16 MiB、REST Case 32 MiB、Normalized Eval Case 32 MiB、Promptfoo Raw Row 64 MiB，解码后的 JSON String Token 16 MiB。REST、Normalized 和 Raw Artifact 不设总文件上限；REST/Normalized 按 Case 流式解析，Raw Retry 只复核登记 Hash、大小和 Descriptor，不重新解析正文。
