...
P4 已落地资源管理 Web。当前只注册 Dashboard 资源数量、Test Suite/Case、Endpoint、LLM、LLM Rubric Prompt 和 Case Analysis Prompt；Run、Report 和 Analysis 页面保持未注册。
...
- [feature-registry.ts](../../apps/web/src/features/feature-registry.ts)：P4 导航与 Dashboard 能力贡献。
...
目标测试覆盖 API Client、请求/响应身份、路由、缓存失效、表单错误映射、刷新恢复、取消、可访问性和五个 Feature 的主路径。源码架构门禁同时限制 `apps`、`packages` 与 `tooling` 的 TypeScript/TSX 文件不超过 1,000 个物理行。
