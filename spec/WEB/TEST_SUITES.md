...
P5 已落地当前 Test Suite 与 Case 资源管理，并在测试集列表显示按创建时间与 ID 确定的最新平台 Run 状态和详情链接；没有平台 Run 时显示空事实。
...
千级 Case 列表保持可用。正在运行使用的测试集禁止删除。所有 Case 写入口呈现一致的校验结果。最近 Run 聚合只包含 `sourceType=PLATFORM`，不把离线导入冒充平台运行。
