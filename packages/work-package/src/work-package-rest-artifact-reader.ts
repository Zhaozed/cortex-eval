...
/** Stream one canonical REST JSONL Artifact without materializing its Case collection. */
export async function* readWorkPackageRestArtifact(
  ...
  const maximumBytes = jsonlTotalByteLimit(
    ...
    WORK_PACKAGE_RUNTIME_LIMITS.restResultCaseBytes
  );
  let lineIndex = 0;
  const caseKeys = new Set<string>();
  let summary: WorkPackageRestArtifactSummary | null = null;
  ...
  for await (const line of readBoundedJsonlLines(input, {
    ...
    try {
      ...
      if (lineIndex === 0) {
        ...
        const header = WorkPackageRestJsonlHeaderV1Schema.parse(line.value);
        if (
        ...
        const record = WorkPackageRestJsonlCaseV1Schema.parse(line.value);
        ...
        if (line.sizeBytes > WORK_PACKAGE_RUNTIME_LIMITS.jsonlControlLineBytes) throw invalid();
        const footer = WorkPackageRestJsonlFooterV1Schema.parse(line.value);
        summary = { completedAt: footer.completedAt, resultSetHash: footer.resultSetHash };
      } else {
        throw invalid();
      }
      lineIndex += 1;
  ...
  if (lineIndex !== expectation.expectedCaseCount + 2 || summary === null) throw invalid();
  return summary;
}
