...
describe("TestSuiteService", () => {
  ...
  it("按稳定 Cursor 与组合条件查询并按 Ordinal 导出完整定义", async () => {
    ...
    expect(firstPage).toMatchObject({
      ok: true,
      page: { nextAfterOrdinal: null, items: [{ caseKey: "case-0" }] }
    });
    expect(
    ...
    expect(await service.queryCases({ suiteId: "suite-1", limit: 0, afterOrdinal: -1 })).toEqual({
      ok: false,
      error: { code: "CASE_QUERY_INVALID", path: "limit" }
    });
    expect(await service.queryCases({ suiteId: "suite-1", limit: 20, afterOrdinal: -1 })).toEqual({
      ok: false,
    ...
    expect(await service.queryCases({ suiteId: "suite-1", limit: 20, afterOrdinal: 1.5 })).toEqual({
      ok: false,
      error: { code: "CASE_QUERY_INVALID", path: "afterOrdinal" }
    });
