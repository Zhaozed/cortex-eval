...
分析可以在报告之后单独导入。分析导入先通过 Execution ID 定位 Run，再校验 Final Case Result Hash 和 Analysis Input Hash；相同 Analysis Input Hash 幂等，不同输入按 Case Analysis Revision 条件更新当前分析。完整 Analysis Case 文件流在主库写事务外消费、校验并写入 owner-only SQLite staging；最终 `BEGIN IMMEDIATE` 只执行确定性依赖/冲突校验、`INSERT ... SELECT` 集合写入和导入身份提交。Busy 重试复用同一份已完成 staging，不重新读取外部文件。
...
输出必须包含 Classification、Confidence、非空结构化 Evidence、Explanation 和 Recommended Action，可选包含一个 Proposal。每项 Evidence 的 `source` 只能指向六个完整 Analysis Input 变量之一，`fieldPath` 是必填的 RFC 6901 Pointer 或 `null`，`conclusion` 非空。字符串 Evidence 不属于当前契约，Contracts、Adapter、Domain、Artifact 与持久化层均不兼容、不转换、不猜测。Analysis Artifact 的错误码是闭合联合，错误文案长度受限；平台导入只信任错误码，并用同时覆盖 Analyzer Case 错误与平台 Analysis 错误的消息解析器重建展示文案。
...
- `/runs/:id/reruns`
- `/runs/:id/analyses`
- `/runs/:id/analyses/:caseKey`
- `/runs/:id/analyses/:caseKey/reject`
- `/runs/:id/analyses/:caseKey/accept`
- `/runs/:id/analyses/:caseKey/edit-and-accept`
- `/work-packages/export`
- `/execution-results/import`
- `/execution-results/analysis/import`
...
P8 在 `/runs` 与 `/runs/:id` 上补充 Report 启动、三阶段 Pipeline 恢复和 Retry/Force，并注册 `/runs/:id/report` 统一展示平台或离线导入报告。P9 注册 `/runs/:id/analysis`：用户显式选择失败/错误/全部可分析 Case、Analyzer、Analysis Prompt 和并发，使用 Cursor 续页读取全部可分析 Case，并遍历 Analyzer 与 Analysis Prompt 的全部配置 Cursor 后提供选择；用户可查看当前版本、分类、模型自评 Confidence、结构化 Evidence、解释、建议与 Proposal，并执行拒绝、接受或编辑后接受。决策冲突后重新读取服务端当前 Analysis，展示其 Revision、Decision 与 Apply Status，同时保留并锁定用户编辑文本，不自动 Rebase。运行时仍以 API 查询重新读取持久事实。
...
- 外部调用不持有 SQLite 事务。
- 应用启动在开放请求前把遗留 Run `RUNNING` 收敛为 `INTERRUPTED`，并把遗留 Analysis `PENDING/RUNNING` 收敛为 `ANALYSIS_INTERRUPTED/ERROR`。
...
- 结构化 Evidence 来源、RFC 6901 路径、字符串拒绝和 Artifact 往返。
- 平台/离线并发、逐 Case 错误隔离、重新分析版本与并发状态冲突。
- Analysis 双遍导入、显式执行身份、幂等替换和完整回滚。
